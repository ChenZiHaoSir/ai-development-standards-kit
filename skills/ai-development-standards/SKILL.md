...
- The orchestrator must run a continuous dispatch loop: after a worker returns, validate the result, update the status board, derive the next executable tasks, and continue dispatching until the project is complete, paused by the user, or blocked by a decision that requires user confirmation.
