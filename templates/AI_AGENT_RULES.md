...
6. 主会话必须持续维护任务队列，子任务完成后验收结果、更新看板并继续派发下一批任务。
7. 只有项目完成、用户暂停或出现必须用户确认的阻塞时，主会话才暂停调度。
8. 不手搓成熟工具可解决的问题，优先使用标准库、框架能力和已安装依赖。
9. 交付前必须运行可用的 lint、typecheck、test、build 或说明未运行原因。
10. 最终回复必须包含规范执行情况、技术选型决策、修改文件、复用处理、验证结果、未执行项和风险。
